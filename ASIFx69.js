module.exports = async ({ api, event }) => {
  const logger = require('./ASIFx69/catalogs/4S1Fc.js')
  
  const configCustom = {
    autosetbio: {
      status: false,
      bio: `prefix : ${global.ASIFx69.PREFIX}`,
  ...
  function notification(config) {
    if (config.status) {
      setInterval(async () => {
        const operator = global.ASIFx69.OPERATOR[0];
        api.sendMessage(`Bot Information\n\nusers : ${global.data.allUserID.length}\nGroups : ${global.data.allThreadID.length}\nOperators : ${global.ASIFx69.OPERATOR.length}\nAdmins : ${global.ASIFx69.ADMINBOT.length}`, operator)
      }, config.time * 60 * 1000)
    }
  }
...
if (á = nam.find(i => i.timer == new Date(Date.now()+25200000).toLocaleString().split(/,/).pop().trim())) global.data.allThreadID.forEach(i => api.sendMessage(r(á.message), i));
  ...
  function autoRestart(config) {
    if(config.status) {
      setInterval(async () => {
        logger(`auto restart is processing, please wait.`, "xnil")
        process.exit(1)
      }, config.time * 60 * 1000)
    }
  }
